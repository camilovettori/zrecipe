-- Fix two data-integrity bugs in merge_ingredients (20260726100000):
--
-- BUG 1: ingredient_supplier_codes.ingredient_id has ON DELETE CASCADE, so
-- deleting the loser silently destroyed all of its supplier product-code
-- mappings instead of transferring them to the keeper.
--
-- BUG 2: is_selected_price was never cleared on the loser's price_history
-- rows before they were repointed to the keeper, so the keeper could end up
-- with two rows flagged is_selected_price = true (its own + the loser's),
-- making resolveIngredientPrice pick an arbitrary one.
...
BEGIN
  ...
  -- Clear is_selected_price on loser's rows BEFORE moving them,
  -- so the keeper's selection isn't corrupted with two selected rows.
  ...
  -- Repoint supplier codes from loser to keeper.
  -- ON CONFLICT: if keeper already has the same (supplier, code), keep keeper's and drop loser's.
  UPDATE ingredient_supplier_codes
  SET ingredient_id = keeper_id
  WHERE ingredient_id = loser_id
    AND NOT EXISTS (
      SELECT 1 FROM ingredient_supplier_codes k
      WHERE k.ingredient_id = keeper_id
        AND k.supplier_id = ingredient_supplier_codes.supplier_id
        AND k.product_code = ingredient_supplier_codes.product_code
    );
  GET DIAGNOSTICS supplier_code_count = ROW_COUNT;

  DELETE FROM ingredient_supplier_codes WHERE ingredient_id = loser_id;
  ...
  RETURN jsonb_build_object(
    ...
    'supplier_codes_moved', supplier_code_count
  );
END;
$$;
...
COMMENT ON FUNCTION merge_ingredients IS
  'Merges the loser ingredient into the keeper ingredient. Transfers
   price history, supplier product codes, and recipe_ingredients FKs,
   optionally merges allergens, applies photo/category choices, then
   deletes the loser. SECURITY INVOKER means RLS applies — users can
   only merge ingredients in their own tenant.';
