...
export default function IngredientCard({ ingredient, onDeleteRequest }: IngredientCardProps) {
  ...
  useEffect(() => {
    // User-uploaded image wins; otherwise resolve from manifest
    const userImage = ingredient.image_url?.startsWith('http') ? ingredient.image_url : null
    if (userImage) {
      setCardImage(userImage)
      return
    }
    findIngredientImage(ingredient.name).then(setCardImage)
  }, [ingredient.image_url, ingredient.name])
