...
export default function SupplierSlideOver({
  ...
  useEffect(() => {
    ...
    setValues({
      ...
      quantityInThousands: supplier?.quantityInThousands ?? false,
    })
  }, [open, supplier])
            ...
            <form onSubmit={handleSubmit} className="flex flex-1 flex-col">
              <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
                ...
                <div className="border-t border-slate-100 pt-4">
                  <p className="text-sm font-semibold text-slate-700">Invoice conventions</p>
                  <div className="mt-3 flex items-start justify-between gap-4">
                    <div>
                      <p className="text-sm font-medium text-slate-700">
                        Quantity uses thousands convention
                      </p>
                      <p className="mt-1 text-xs text-slate-500">
                        Turn this on for packaging suppliers (Atlas, Kingfisher, McKays) where
                        invoice quantity 0.250 means 250 units, and unit price 375.000 means
                        €0.375 per unit. When on, extracted values are corrected automatically.
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() =>
                        setValues((current) => ({
                          ...current,
                          quantityInThousands: !current.quantityInThousands,
                        }))
                      }
                      className={cn(
                        'relative inline-flex h-5 w-9 items-center rounded-full transition-colors flex-shrink-0',
                        values.quantityInThousands ? 'bg-emerald-500' : 'bg-gray-200'
                      )}
                    >
                      <span
                        className={cn(
                          'inline-block h-3.5 w-3.5 rounded-full bg-white shadow-sm transition-transform',
                          values.quantityInThousands ? 'translate-x-[18px]' : 'translate-x-[3px]'
                        )}
                      />
                    </button>
                  </div>
