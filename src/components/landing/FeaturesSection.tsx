...
type Accent = 'emerald' | 'amber' | 'terra'

const ACCENT_STYLES: Record<Accent, { chip: string; chipHighlight: string; cardHighlight: string }> = {
  emerald: {
    chip: 'bg-primary-50 text-primary-600',
    chipHighlight: 'bg-primary-600 text-white',
    cardHighlight: 'bg-gradient-to-br from-primary-50 to-primary-100/50 border border-primary-100',
  },
  amber: {
    chip: 'bg-accent-50 text-accent-600',
    chipHighlight: 'bg-accent-500 text-white',
    cardHighlight: 'bg-gradient-to-br from-accent-50 to-accent-100/50 border border-accent-200',
  },
  terra: {
    chip: 'bg-brand-terra/10 text-brand-terra',
    chipHighlight: 'bg-brand-terra text-white',
    cardHighlight: 'bg-gradient-to-br from-brand-terra/10 to-brand-terra/5 border border-brand-terra/20',
  },
}
          ...
          {GROUPS.map((group) => (
            <Reveal key={group.heading}>
              ...
              <div className={`mt-8 grid gap-6 sm:grid-cols-2 ${
                group.features.length === 4 ? 'lg:grid-cols-2' : 'lg:grid-cols-3'
              }`}>
                {group.features.map(({ Icon, title, description, accent, highlight }) => {
                  const styles = ACCENT_STYLES[accent]
                  const shouldSpan = group.features.length === 5 && highlight
                  return (
                    <div
                      key={title}
                    ...
                    >
                      <div
                        className={`flex items-center justify-center rounded-xl ${
                          highlight ? `h-14 w-14 ${styles.chipHighlight}` : `h-12 w-12 ${styles.chip}`
                        }`}
                      >
                        <Icon className={highlight ? 'h-7 w-7' : 'h-6 w-6'} />
                      </div>
